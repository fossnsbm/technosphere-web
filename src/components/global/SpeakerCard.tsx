...
const SpeakerCard = ({ photo, speaker, jobTitle, company, link }: AppState) => {
  const styles = {
    speakerCard: {
      minHeight: 280,
      minWidth: 280,
    ...
    <Grid item xs={12} sm={6} lg={3} justifyContent="center" display="flex">
      <Box className="speakerCard" sx={styles.speakerCard}>
        <Grid container justifyContent="space-between">
          ...
          <Grid item>
            <Typography className="speaker_job">{jobTitle}</Typography>
            <Typography className="speaker_job">{company}</Typography>
          </Grid>
        </Grid>
        <Grid xs="auto">
          <Grid item>
